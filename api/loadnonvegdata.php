	...
	if ($result->num_rows > 0) {
	    ...
	    while($row = $result->fetch_assoc()) {
	      //echo $row["product_id"].$row["resturant_id"].$row["category"].$row["name"].$row["price"].$row["image"].$row["image_type"];	
			...
			$name = $data["name"];
			if($category == "NON-VEG" or $category =="nonveg" or $category=="NONVEG" or $category=="Non Veg"){
				++$countitems;
				if($image_type == "3d"){
						echo  "<li class=\"non-veg product\">
		    					<div class=\"details\"><h2>$name</h2><p class=\"price\">$price</p></div>
		    					<div class=\"logo\"><img src='' alt='$type $productname' /></div>
		    					<div class=\"photo\"><img src='$image' alt='' /></div>
		  						</li>";
				}else{
					  	echo "<li class=\"non-veg product\">
		    					<div class=\"details\"><h2>$name</h2><p class=\"price\">$price</p></div>
		    					<div class=\"logo\" ><img src='' alt='NON-VEG $productname' /></div>
	    ...
	    if($countitems%6 != 0){
	    	...
	    	while($countitems--){
	    		echo "<li class=\"non-veg product\"><div class=\"details\"><h2></h2><p class=\"price\"></p></div><div class=\"logo\"><img src='' alt='' /></div><div class=\"photo\"><img src='' alt='' /></div></li>";
	    	} 
	    }
